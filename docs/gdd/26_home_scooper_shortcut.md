...
## 6. 2026-04-21 收益券整合更新

- 首頁鏟屎快捷入口會讀取 `GameState.get_party_cheer_coupon_count()` 作為收益券持有量。
- 當收益券數量大於 `0` 時，外層 `清理貓砂盆 HH:MM:SS` 按鈕顯示紅點。
- 開啟清理 Dialog 後，`使用收益券(...)` 按鈕也會顯示紅點。
- `使用收益券(...)` 文案改為直接顯示目前剩餘張數，例如 `使用收益券(3)`。
- 使用收益券成功後，若背包內仍有下一張收益券，`使用收益券(...)` 按鈕會進入 2 秒冷卻，之後才可再次使用。
- `使用收益券(...)` 的 2 秒冷卻需顯示覆蓋層與倒數秒數，讓玩家能明確感知冷卻正在遞減，而不是只做 disabled 鎖定。
- Home reward float 目前位於高於 `DialogManager` Dialog 的 canvas layer，因此在放置獎勵 Dialog 中使用收益券成功後，可直接顯示在 Dialog 之上，不需要先關閉 Dialog。
- 使用收益券成功後，獎勵呈現改為共用一般掛機領取的 home reward float，不再用成功 toast 彙總道具。
- 使用收益券成功後，前端以 API 回傳的 `walletSnapshot` 與 `rewards` 為準刷新券數與獎勵顯示，不再本地手動扣券。
- 當放置收益累積時間達 `4` 小時以上時，外層 `清理貓砂盆 HH:MM:SS` 按鈕與 Dialog 內的 `領取獎勵` 按鈕都需要顯示紅點。
