...
- 按鈕位於主畫面戰鬥 HUD 的技能欄標頭列，位置在 `加速` 按鈕左側
- **不足一分鐘時**：按鈕 disabled，顯示「清理貓砂盆」，並沿用強化頁「屬性變更」的 disabled 配色
- **滿一分鐘以上**：按鈕 enabled，顯示累積時長，格式 `清理貓砂盆 HH:MM:SS`
  - 例：「清理貓砂盆 05:32:07」→ 可領取 5 小時 32 分鐘的獎勵，07 秒繼續累積
- 可清理時按鈕使用與強化頁「屬性變更」一致的可用配色
- 按鈕每秒更新時長顯示
- 開啟清理 Dialog 後，於「領取獎勵」上方提供 `使用收益券(目前持有數)` 入口
- 當玩家身上持有至少 1 張收益券時：
  - 主畫面 `清理貓砂盆 HH:MM:SS` 按鈕顯示紅點
  - Dialog 內的 `使用收益券(...)` 按鈕也顯示紅點
- 收益券按鈕數量直接顯示 `partyCheerCouponCount` 的當前剩餘值，不再顯示 `/5` 上限文案
...
| 掛機物品加成 | 掛機產出數量提升 X% |
| 掛機時間上限加成 | 最大累積時間延長 |
| 解鎖 1.5x / 2x / 3x 推進倍率 | 主畫面 `加速` 按鈕可在免費加速時套用目前已解鎖的最高倍率 |
| 免費加速 30 分鐘 | 未解鎖額外倍率時，主畫面 `加速` 按鈕提供 1.2x、持續 30 分鐘、免費一次；啟動後顯示 `<倍率>加速中 MM:SS` |
| 跳過按鈕 | 目前作為管理員 / 測試權限功能，不列為一般玩家可解鎖能力 |
...
- 已達成但未領取時：
  - 成就卡片顯示可領取狀態
  - 成就 Tab 按鈕顯示提示
- 成就達成後不再跳出額外通知 Dialog；玩家可直接前往 `鏟屎官 > 成就` 查看並領取獎勵
...
## 12-X 2026-04 裝備卡 UI/互動補充

本節補充目前 Scooper 裝備 Tab 已實作的卡片結構、狀態文案與互動規則，作為前端與企劃對齊基準。

### 裝備卡固定版型

- 左欄為裝備圖片，維持三行高度的視覺主欄位。
- 右欄固定為四段資訊：
  - 第 1 行：品項名稱、等級、狀態標籤。
  - 第 2 行：效果描述；右側可顯示狀態補充小字。
  - 第 3 行：經驗條。
  - 第 4 行：主操作按鈕。
- 未持有與未解鎖也必須保留相同版型，不可改成獨立置中的特殊按鈕版面。
...
- 可升級：
  ...
  - 按鈕：開始逗貓
- 損壞中：
  - 右側小字：主子破壞了裝備，重購需xxxx金幣
  - 按鈕：修理裝備
- 需治療：
  - 右側小字：主子累到生一場病，就醫需金幣xxxx
  - 按鈕：診所就醫
- 已滿級：
  - 右側小字：主子不想再看到這玩意了，需鏟屎官提升等級
  - 按鈕：目前等級上限
  - 按鈕需為 disabled。
- 未解鎖：
  - 卡面中央深色遮罩文字：鏟屎官等級X解鎖
  - 右側小字：主子看起來很想要，需鏟屎官達X級
  - 按鈕：購買裝備
  - 按鈕需為 disabled。
- 未持有：
  - 右側小字：主子看起來很想要，只需要金幣xxxx
  - 按鈕：購買裝備
  - 按鈕可操作。
...
- 已持有裝備顯示實際 EXP x/y。
- 未持有與未解鎖裝備也保留經驗條，但固定顯示  /0。
- 已滿級時不再於效果右側重複顯示 EXP(MAX) 小字。
- 經驗條配色統一參考強化頁的綠色 ProgressBar 樣式。

### 金幣不足統一流程

凡是操作會消耗金幣時，都必須先判斷玩家當前金幣是否足夠。適用狀態如下：
...
- 右側小字：無法逗主子開心，金幣需 xxxx
- 按鈕文字：金幣不足
- 按鈕字色改為紅色
- 按鈕 disabled，不送出 request

### 連點防護

- 裝備 Tab 操作送出後，整體按鈕需進入 inflight 鎖定，避免短時間連點造成重複 request。
- 成就、記憶、Mail 的領取操作也補上同類型 inflight 鎖定。
- 共用 API request pool 不再使用前端 pending queue 排隊；無可用 slot 時直接回傳 busy，避免畫面長時間卡住。
