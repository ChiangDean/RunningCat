...
- 上方 HUD 第三格顯示 `戰力指數`，圖示使用 `evil_cat_power_icon.png`，數值由前端使用 bootstrap 回傳的 `combatPowerWeights` 與目前快取數值本機計算；點擊該區塊可開啟 `StatsScene` 查看目前影響數值的來源。

### 戰力指數

- `戰力指數` 不再由沙發或洗澡玩法加總，改由前端使用後端 bootstrap 回傳的權重表本機計算。
- 後端以 `combat_power_weight_catalog` 定義各項戰鬥數值的權重、基礎分與啟用狀態，Admin Catalog 的 `combat-power` section 可直接調整；bootstrap 只回傳權重 catalog，不在後端計算戰力。
- 戰力計算以目前 Boss 隊伍為基礎，套用貓咪基礎值、升級、品階、特殊貓糧、被動技能，以及裝備、記憶、寶物等 `StatsScene` 會顯示的戰鬥數值加成。
- 所有會影響戰力的本機快取資料更新後，前端用目前 `combatPowerWeights` 重新計算主戰力。沙發、洗澡等限時活動分數不覆寫主戰力。
- 若重算後 `combatScore` 上升或下降，前端在畫面下方顯示一行非阻塞戰力變化提示，不使用 toast，也不顯示會遮住操作的遮罩。

### 限時活動玩法

- 沙發與洗澡玩法改定位為獨立活動紀錄，未開放前不出現在常駐活動入口，也不影響 `戰力指數`。
- 沙發活動保留 30 秒演出與累積總傷害的呈現；洗澡活動保留分數隨時間增加、貓咪受擊亂竄的呈現。
- 這兩個玩法未來可改為真正戰鬥或耐久玩法；命名與 UI 文案應避免稱為「戰力試煉」。
...
- 動作完成後的資料同步、bootstrap-backed 頁面自我修復、戰力重算等背景刷新應使用 silent request，不顯示共用 loading overlay；只在登入、首次載入、明確阻塞整個頁面的操作才顯示共用 loading overlay。
