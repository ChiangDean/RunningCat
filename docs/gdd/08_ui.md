...
- 上方 HUD 第三格顯示 `戰力指數`，數值來源為後端保存的戰力試煉最高紀錄；點擊該區塊可開啟 `CombatTrialScene`。

### 戰力試煉

- `戰力試煉` 包含 `沙發試煉` 與 `洗澡試煉`。
- `沙發指數`：以目前 Boss 隊伍壓縮模擬 60 秒攻擊同一張無限 HP 沙發，分數代表總輸出。
- `洗澡指數`：每 0.1 秒承受一次全體洗澡傷害，傷害逐步提高，直到 60 秒或全軍覆沒；分數代表隊伍承受逐步加壓的能力。
- `戰力指數 = 最高沙發指數 + 最高洗澡指數`。
- 試煉結算送至 `POST /api/combat-trial/score`，後端只保存同類試煉的歷史最高分，bootstrap 回傳 `sofaScore`、`bathScore`、`combatScore` 與 `combatTrialVersion`。
