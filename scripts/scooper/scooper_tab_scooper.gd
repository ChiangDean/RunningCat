...
func _make_equip_card(scene: Control, item: Dictionary) -> Control:
	...
	if equipment_icon != null:
		header.add_child(AssetResolver.create_icon_rect(equipment_icon, Vector2(52.0, 52.0)))

	var content_block: VBoxContainer = VBoxContainer.new()
	content_block.add_theme_constant_override("separation", 5)
	content_block.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	header.add_child(content_block)

	var title_lbl: Label = Label.new()
	title_lbl.add_theme_font_size_override("font_size", 22)
	title_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	content_block.add_child(title_lbl)

	var desc_lbl: Label = Label.new()
	desc_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	desc_lbl.add_theme_font_size_override("font_size", 16)
	desc_lbl.add_theme_color_override("font_color", Color(0.84, 0.84, 0.82, 1.0))
	content_block.add_child(desc_lbl)

	var action_btn: Button = Button.new()
	action_btn.custom_minimum_size = Vector2(160.0, 46.0)
	...
	if locked:
		...
		desc_lbl.text = "%s\n%s" % [UiText.SCOOPER_EQUIPMENT_UNLOCK_AT % unlock_lv, _bonus_desc(item, 0)]
	...
	if not owned:
		...
		action_btn.text = UiText.SCOOPER_EQUIPMENT_PURCHASE % UPGRADE_COST
		action_btn.disabled = true
		_add_unlock_overlay(scene, panel, int(item.get("purchaseCost", 0)), equip_id, name_str)
		return panel
	...
	if broken:
		...
		action_btn.text = UiText.SCOOPER_EQUIPMENT_REPAIR % int(item.get("repairCost", 0))
		_register_cooldown_button(scene, action_btn, equip_id, "repair")
		action_btn.disabled = _is_action_cooling(scene, equip_id, "repair")
	...
	if treat_mode:
		...
		desc_lbl.text = "%s\n%s" % [sick_cat_name, _bonus_desc(item, maxi(level, 1))]
		action_btn.text = UiText.SCOOPER_EQUIPMENT_TREAT % int(item.get("treatCost", 0))
		_register_cooldown_button(scene, action_btn, equip_id, "treat")
		action_btn.disabled = _is_action_cooling(scene, equip_id, "treat")
		action_btn.pressed.connect(func() -> void:
			_do_equipment_action(scene, "treat", equip_id)
		)
		return panel

	action_btn.text = UiText.SCOOPER_EQUIPMENT_UPGRADE % UPGRADE_COST
	...
	var can_upgrade: bool = can_afford_upgrade and level < scooper_lv and cooldown_ready
	action_btn.disabled = not can_upgrade
	...
	if not can_afford_upgrade:
		...
		desc_lbl.text = "%s\n%s" % [desc_lbl.text, UiText.SCOOPER_EQUIPMENT_UPGRADE_DISABLED_GOLD]
	elif level >= scooper_lv:
...
func _add_unlock_overlay(scene: Control, panel: PanelContainer, unlock_cost: int, equip_id: int, item_name: String) -> void:
	...
	unlock_btn.text = UiText.SCOOPER_EQUIPMENT_UNLOCK % unlock_cost
	...
	unlock_btn.disabled = scene.GameState.player_data.gold < unlock_cost
	...
	unlock_btn.pressed.connect(func() -> void:
		...
		scene.DialogManager.show_confirm(
			...
			UiText.SCOOPER_EQUIPMENT_UNLOCK_CONFIRM_BODY % [unlock_cost, item_name],
			func() -> void:
				if not _is_scene_alive(scene):
					return
