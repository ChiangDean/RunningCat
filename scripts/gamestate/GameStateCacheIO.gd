...
## 靜態 JSON 快取讀寫工具：catalog、scooper、config 三個快取目錄
...
static func save_catalog(catalog_name: String, data: Array) -> void:
	...
	if file == null:
		push_error("GameStateCacheIO: 無法寫入 catalog 快取 " + path)
		return
...
static func save_scooper(cache_name: String, data: Variant) -> void:
	...
	var file := FileAccess.open(path, FileAccess.WRITE)
	if file == null:
		push_error("GameStateCacheIO: 無法寫入 scooper 快取 " + path)
		return
	file.store_string(JSON.stringify(data))
	file.close()
...
static func save_config(cache_name: String, data: Variant) -> void:
	...
	var file := FileAccess.open(path, FileAccess.WRITE)
	if file == null:
		push_error("GameStateCacheIO: 無法寫入 config 快取 " + path)
		return
	file.store_string(JSON.stringify(data))
	file.close()
